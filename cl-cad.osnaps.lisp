...
(defun get-center-coordinates (cd)
      ...
      (cond ((equal (getf cd :title) :circle)
		 ...
		 (progn
		   ...
		   (if (> *snap-distance* (get-snap-length (* *scroll-units* (getf cd :x1)) 
							   (* *scroll-units* (getf cd :y1))))
		       (progn
			   (setf *snap-distance* (get-snap-length (* *scroll-units* (getf cd :x1)) 
								  (* *scroll-units* (getf cd :y1))))
			   (setf *snap-x1* (getf cd :x1) *snap-y1* (getf cd :y1))))))))))

(defun get-end-coordinates (cd)
  (if (equal *osnap-end* t)
      (cond ((equal (getf cd :title) :line)
	     (cond ((>= *osnap-area-delta* (get-snap-length (* *scroll-units* (getf cd :x1)) 
							    (* *scroll-units* (getf cd :y1))))
		    (coordinate-light-square (* *scroll-units* (getf cd :x1)) 
					     (* *scroll-units* (getf cd :y1))))
		   ((>= *osnap-area-delta* (get-snap-length (* *scroll-units* (getf cd :x2)) 
							    (* *scroll-units* (getf cd :y2))))
		    (coordinate-light-square (* *scroll-units* (getf cd :x2)) 
					     (* *scroll-units* (getf cd :y2)))))))))

(defun get-midpoint-coordinates (cd)
  (if (equal *osnap-midpoint* t) 
      (cond ((equal (getf cd :title) :line)
	     (if (>= *osnap-area-delta* (get-snap-length (* *scroll-units* (/ (+ (getf cd :x2) 
										 (getf cd :x1)) 2))
							 (* *scroll-units* (/ (+ (getf cd :y2) 
										 (getf cd :y1)) 2))))
		 (progn
...
(defun get-point-coordinates (cd)
...
(defun osnap-window (parent-window)
  (within-main-loop
   (let ((window (make-instance 'gtk-window 
     ...
     (gobject:g-signal-connect button-oscenter "clicked" (lambda (a) (declare (ignore a)) 
								 (if (equal *osnap-center* t) 
								     (setf *osnap-center* nil) 
								     (setf *osnap-center* t))))
     (gobject:g-signal-connect button-osend "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-end* t) 
								  (setf *osnap-end* nil) 
								  (setf *osnap-end* t))))
     (gobject:g-signal-connect button-osins "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-insert* t) 
								  (setf *osnap-insert* nil) 
								  (setf *osnap-insert* t))))
     (gobject:g-signal-connect button-osint "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-intersection* t) 
								  (setf *osnap-intersection* nil) 
								  (setf *osnap-intersection* t))))
     (gobject:g-signal-connect button-osmid "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-midpoint* t) 
								  (setf *osnap-midpoint* nil) 
								  (setf *osnap-midpoint* t))))
     (gobject:g-signal-connect button-osnea "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-nearest* t) 
								  (setf *osnap-nearest* nil) 
								  (setf *osnap-nearest* t))))
     (gobject:g-signal-connect button-osnod "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-point* t) 
								  (setf *osnap-point* nil) 
								  (setf *osnap-point* t))))
     (gobject:g-signal-connect button-osper "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-perpendicular* t) 
								  (setf *osnap-perpendicular* nil) 
								  (setf *osnap-perpendicular* t))))
     (gobject:g-signal-connect button-osqua "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-quadrant* t)
								  (setf *osnap-quadrant* nil)
								  (setf *osnap-quadrant* t))))
     (gobject:g-signal-connect button-ostan "clicked" (lambda (a) (declare (ignore a)) 
							      (if (equal *osnap-tangent* t)
								  (setf *osnap-tangent* nil)
								  (setf *osnap-tangent* t))))
     (gobject:g-signal-connect button-ostrack "clicked" (lambda (a) (declare (ignore a)) 
								(if (equal *osnap-track* t)
								    (setf *osnap-track* nil)
								    (setf *osnap-track* t))))
     (gobject:g-signal-connect button-osgrid "clicked" (lambda (a) (declare (ignore a)) 

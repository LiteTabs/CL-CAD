...
(defstruct current-layers layer-name)
...
(defun main ()
  ...
  (within-main-loop
   (let* ((app (make-app))
	  ...
	  (model-layer (make-instance 'array-list-store))
	  (layers-combo (make-instance 'combo-box :model model-layer))
	  ;;;menu
     ...
     (container-add toolbar layers-combo)
     ...
     (store-add-column model-layer "gchararray" #'current-layers-layer-name)
     (liner model-type model-width)
     (dolist (cd *current-draw*)
	(if (equal (getf cd :title) :layer)
	    (store-add-item model-layer (make-current-layers :layer-name (getf cd :layer-name)))))
     ...
     (let ((renderer (make-instance 'cell-renderer-text :text "A text")))
        (cell-layout-pack-start layers-combo renderer :expand t)
        (cell-layout-add-attribute layers-combo renderer "text" 0))
     ...
     (gobject:g-signal-connect draw-area "motion-notify-event" (lambda (widget event)
								 (declare (ignore widget))
			  					 (setf *current-x* (round (gdk:event-motion-x event))
								       *current-y* (round (gdk:event-motion-y event)))
     ...
     (gobject:g-signal-connect menuitem-editor "activate" (lambda (w) (declare (ignore w)) (code-editor)))
     (gobject:g-signal-connect menuitem-osnap "activate" (lambda (b) (declare (ignore b)) (osnap-window (app-main-window app))))
     (gobject:g-signal-connect menuitem-export "activate" (lambda (b) (declare (ignore b)) (cb-export app)))
...
(defun liner (model-type model-width)

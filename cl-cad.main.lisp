  ...
  (within-main-loop
   (let* ((app (make-app))
     ...
     (gobject:g-signal-connect draw-area "button-press-event" (lambda (widget event)
								(declare (ignore widget))
								(cond
								  ((= 1 (gdk:event-button-button event)) (if (equal *end* 0) (setf *x* *current-x* *y* *current-y*)))
								  ((= 2 (gdk:event-button-button event)) (setf *end* 0) (setf *x3* *current-x* *y3* *current-y*))
     ...
     (gobject:g-signal-connect draw-area "button-release-event" (lambda (object event)
								  ...
								  (setf *x4* *current-x* *y4* *current-y*)))

...
;(defun on-draw-area-scroll (event)
;  (if (equal (gdk:event-scroll-direction event) :UP)
;      (1+ *scroll-units*)
;	((equal (event-scroll-direction event) :DOWN)
;      (1- *scroll-units*)))
   ...
   (let* ((w (make-instance 'gtk-window :title "CL-CAD" :type :toplevel :window-position :center :default-width 1024 :default-height 600 :app-paintable t))
     ...
     (gobject:g-signal-connect draw-area "motion-notify-event" (lambda (widget event)
								 (declare (ignore widget))
			  					 (setf *x* (gdk:event-motion-x event)
     ...
     (gobject:g-signal-connect draw-area "configure-event"
                        (lambda (widget event)
                          (declare (ignore widget event))
                          (widget-queue-draw draw-area)))
     (gobject:connect-signal draw-area "realize"
			     (lambda (object)
			       (declare (ignore object))
			       (pushnew :button-press-mask
					(gdk:gdk-window-events (widget-window draw-area)))))
...
;(defun winner ()
;  (within-main-loop
;   (let ((w (make-instance 'gtk-window :title "test"))
;	 (d (make-instance 'drawing-area :height-request 100 :width-request 100)))
;     (container-add w d)
;     (gobject:connect-signal d "expose-event" (lambda (widget event)
;						(cl-cairo2::with-gtk-context (let (c (cl-cairo2:pointer (widget-window d)))
;									       (cl-cairo2:rectangle 10 10 20 20 c)))))
;     (gobject:g-signal-connect w "destroy" (lambda (b) (declare (ignore b)) (leave-gtk-main)))
;     (widget-show w))))